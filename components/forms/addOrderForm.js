  ...
  const domString = `
    <form id="${obj.firebaseKey ? `update-order--${obj.firebaseKey}` : 'submit-order'}" class="mb-4">
      <div class="form-group">
        ...
        <select id="orderType" style="width: 200px; text-align: center; border-radius: 5px; margin-bottom: 10px;" class="mt-3" required>
          <option value="None">Select an Order Type</option>
          <option value="Phone">Phone</option>
          <option value="In-Person">In-Person</option>
        </select>
      </div>
      <div class="form-group">
        <label style="color: slategrey; margin-bottom: 5px;" for="title">Order Name</label>
